...
// import { GenerateComponent } from '../../data-connector/generate/generate.component';
import { getDatasetId } from 'src/app/helpers/functions';

@Component({
  selector: 'app-pipedrive-report',
  templateUrl: './pipedrive-report.component.html',
  styleUrls: ['./pipedrive-report.component.scss']
})
export class PipedriveReportComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private dialog: MatDialogRef<PipedriveReportComponent>, private router: Router,
      ...
      .subscribe({
        next: (res) => {
          if (res['status'] === true) {
            ...
            this.finalList = this.connectorList.filter((item: any) => item.service == 'pipedrive' && item.status === 'connected');
  ...
  workspaceReport(row: any) {
    ...
    formData.append('source_workspace', environment.pipedriveTemplateName);
    // formData.append('target_workspace', `${this.email} - ${row.nameOfConnector}`);
    formData.append('target_workspace', `${this.email} - pipedrive - ${Date.now()} `);
      ...
      .subscribe({
        next: (res) => {
          if (res['status'] === true) {
            ...
            this.newValue = row.nameOfConnector
  ...
  updateDataset() {
    ...
    let testing = JSON.stringify([
      {
        "name": "dataset_name",
        "newValue": getDatasetId('pipedrive', this.newValue)
      },
      {
